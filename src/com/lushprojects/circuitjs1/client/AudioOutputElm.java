...
//	    int frac = (int)Math.round(Math.max(sampleStep*33000, 1));
        ...
        Wav.prototype.getWavInt16Array = function(buffer){
        	...
        	// RIFF 大小 = 文件总大小 - 8。头部为 46 字节（23 个 Int16 字），
        	// 因此为 (2*buffer.length + 46) - 8 = 2*buffer.length + 38。
        	intBuffer[2] = (2*buffer.length + 38) & 0x0000ffff; // RIFF 大小
        	intBuffer[3] = ((2*buffer.length + 38) & 0xffff0000) >> 16; // RIFF 大小
        	...
        	intBuffer[11] = this._channels; // 声道数：2
        	...
        	intBuffer[16] = 0x0002; // 块对齐（单声道 16 位 = 2 字节）
        	intBuffer[17] = 0x0010; // 每个样本的位数
	...
	// 如有旧的 blob 和音频对象则将其移除。我们应该在音频播放完毕后做这件事，但这样更简单
	if (oldblob) {
...
//        console.log(url);
	...
	audio.play();
        ...
        void play() {
            ...
            // 防止信号为恒定值（max == min）时出现除以零 / 无穷大的情况
            double amp = max+adj;
            double mult = (amp == 0) ? 0 : (.25*32766)/amp;
