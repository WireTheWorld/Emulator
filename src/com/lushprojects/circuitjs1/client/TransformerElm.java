...
package com.lushprojects.circuitjs1.client;
	...
	void reset() {
	    // 需要在此处设置电流源的值,以防某个节点是节点 0。在这种情况下,
	    // 分析电路时可能在 startIteration() 被调用之前,
	    // 就从 setNodeVoltage() 调用了 calculateCurrent()
	...
	void stamp() {
	    ...
	    //
	    // 对于向后欧拉(backward euler),
	    //
	    //   i1(t2) = i1(t1) + a1 dt v1(t2) + a2 dt v2(t2)
	    //
	    // 因此电流源的值就是 i1(t1),并且我们对
	    // 电阻和 VCCS 使用 dt 而不是 dt/2。
	    //
	    // 第一个绕组从节点 0 到 2,第二个绕组从 1 到 3
	    ...
	    a1 = l2*deti*ts; // 我们在此处将 dt/2 乘入 a1..a4
	    a2 = -m*deti*ts;
	    a3 = -m*deti*ts;
	    a4 = l1*deti*ts;
