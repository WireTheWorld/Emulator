...
class TestPointElm extends CircuitElm {
    ...
    final int TP_PER = 7;
    final int TP_PWI = 8;
    final int TP_DUT = 9; // 占空比（高电平与低电平时间之比）
    int zerocount=0;
    ...
    double frequency=0;
    double period=0;
    double binaryLevel=0;//0 或 1 - 使用 double 是因为我们只把 double 传回网页
    double pulseWidth=0;
    double dutyCycle=0;
    ...
    public TestPointElm(int xa, int ya, int xb, int yb, int f,
             StringTokenizer st) {
        super(xa, ya, xb, yb, f);
        meter = new Integer(st.nextToken()).intValue(); //从保存的 dump 中获取仪表类型
    }
    ...
    void draw(Graphics g) {
        ...
        setBbox(point1, lead1, 0);
                    drawCenteredText(g, s, x2, y2, true); //绘制标签 TPx
        //绘制所选值
        switch (meter) {
            case TP_VOL:
...
//                s = "percent:"+period + " " + sim.timeStep + " " + sim.simTime + " " + sim.getIterCount();
            ...
            drawCenteredText(g, s, x2, y2+12, true); //绘制所选值 TPx
	...
	lastStepCount = sim.timeStepCount;
        ...
        if (volts[0]<maxV && increasingV){//方向改变，V 现在下降 - 位于波形起点
            lastMaxV=maxV; //捕获上一次的最大值 
            //捕获之间的时间
        ...
        if (volts[0]<minV && decreasingV){ //V 下降，用 V 跟踪最小值
            minV=volts[0];
            increasingV=false;
            decreasingV=true;
        }
