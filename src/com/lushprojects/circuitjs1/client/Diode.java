...
// 可嵌入其他元件中的二极管。串联电阻在 DiodeElm 中处理，不在此处。
class Diode {
    int nodes[];
    CirSim sim;
...
//	sim.console("setup " + leakage + " " + zvoltage + " " + model.emissionCoefficient + " " +  vdcoef);
    ...
    double vscale;
    // 除以 vscale 的乘法等价形式（为速度考虑）。
    double vdcoef;
    // 齐纳击穿曲线由一条更陡峭的指数曲线表示，类似于理想的
    // Shockley 曲线，但经过了翻转和平移。该曲线消除了 emcoef
    // 的平缓影响，用 vt 和 vzcoef 取代 vscale 和 vdcoef。
    // vzcoef 是除以 vt 的乘法等价形式（为速度考虑）。
    static final double vzcoef = 1 / vt;
    // 用户指定的正向压降和齐纳电压二极管参数。
    double fwdrop, zvoltage;
    // 由用户指定的正向压降计算出的二极管电流比例因子。
    double leakage;
    // 由用户指定的齐纳电压计算出的齐纳击穿指数电压偏移量。
    double zoffset;
    // 用于限制普通二极管和齐纳击穿指数的临界电压。
    double vcrit, vzcrit;
    double lastvoltdiff;
	...
	if (voltdiff >= 0 || zvoltage == 0) {
	    ...
	    // 对于反向偏置的齐纳二极管，用类似于理想 Shockley 曲线的
	    // 指数函数来模拟齐纳击穿曲线。（真实的击穿曲线
	    // 不是简单的指数函数，但这种近似应该没问题。）
